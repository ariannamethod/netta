...
world. Netta's own continuations may change experiential state and action policy,
but they never become source evidence merely because she generated them.

### Inputs and outputs

Input is plain text. The current release uses a corpus-built word/punctuation
vocabulary and reports unknown interactive tokens rather than silently teaching
them as facts. Output is a corpus-grounded continuation. Interactive exchanges
...
beat a matched control on the game Netta is built to play: coherence.
