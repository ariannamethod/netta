...
Her first measurement under [`SPEECH_COURT.md`](SPEECH_COURT.md) came back an honest SPEECH FAIL: every stream beat ignorance, and every stream also crossed the frozen anti-copy line — coverage between 0.9657 and 1.0 against a void threshold of 0.50. The parrot risk the court exists to catch, caught on the first try. Then the corridor law landed ([`MOUTH_PROTOCOL_A2.md`](MOUTH_PROTOCOL_A2.md)): a lived support holding exactly one continuation is a step along the tape, not a choice, and after a few such steps the mouth must descend to a support with a real branch. Under that law the plain mouth passed the sealed census — the reader printed its first literal *SPEECH PASS: the mouth speaks below ignorance and above copying* — and the price of speech rose from near-zero to a real number of bits per byte, which is what choosing costs. The citizens triple is not clean yet, and that stands in the log as measured, not smoothed. A second, independent hand — one that rebuilds the unit inventory from scratch and shares no code with the mouth — stands beside every measurement.

## what isn't proven

Court 4 earned one relation in one drawn class on one unseen base, not transfer at scale, and nobody in this repo gets to call it that. Body 1's plain mouth has cleared the census; the advised triple has not yet, and the word coherence still belongs to the court alone. Read [`NETTALOG0.md`](NETTALOG0.md) for the receipts behind every sentence here.
