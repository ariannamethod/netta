...
static const char *mode_names[AR_MODES] = {"slow", "fast", "adaptive", "witness",
...
static void charge(Stats *s, ARMode mode, double cold, double live,
    ...
    double bound = mode == AR_CEILING ? log2(33.0) :
                   mode == AR_WITNESSED_CEILING ? log2(1025.0) :
                   mode == AR_FAST ? 10.0 : 16.0;
...
int main(int argc, char **argv) {
    ...
    fputs("\tadaptive_e_before\tadaptive_e_after"
          ...
          "\twitnessed_ceiling_live\twitnessed_ceiling_odds_before"
          "\twitnessed_ceiling_slow_used\twitnessed_ceiling_odds_after"
          "\twitnessed_ceiling_w_before\twitnessed_ceiling_w_after"
          "\twitnessed_ceiling_clipped\twitnessed_ceiling_cap_after\n", stdout);
    size_t expected_t = 0;
    char line[512];
    while (fgets(line, sizeof(line), stdin)) {
        ...
        if (before[AR_WITNESSED_CEILING].odds > 10.0 ||
            state[AR_WITNESSED_CEILING].odds > next_limit)
            fail("witnessed ceiling odds bound");
        if (before[AR_WITNESSED_CEILING].evidence != before[AR_WITNESS].evidence ||
            state[AR_WITNESSED_CEILING].evidence != state[AR_WITNESS].evidence ||
            slow_used[AR_WITNESSED_CEILING] != slow_used[AR_WITNESS])
            fail("witnessed ceiling/witness clock identity");
        ...
        printf("\t%.17g\t%.17g\t%d\t%.17g\t%.17g\t%.17g\t%d\t%.17g\n",
               live[AR_WITNESSED_CEILING], before[AR_WITNESSED_CEILING].odds,
               slow_used[AR_WITNESSED_CEILING], state[AR_WITNESSED_CEILING].odds,
               before[AR_WITNESSED_CEILING].evidence,
               state[AR_WITNESSED_CEILING].evidence,
               clipped[AR_WITNESSED_CEILING], next_limit);
