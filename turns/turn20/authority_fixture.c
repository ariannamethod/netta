...
static const char *const names[AR_MODES] = {"slow", "fast", "adaptive", "witness",
                                           "ceiling", "witnessed_ceiling"};
...
int main(void) {
    ...
    unsigned clips = 0, low_clips = 0, high_clips = 0;
    ...
    for (int t = 0; t < 33; t++) {
        ...
        for (int m = 0; m < AR_MODES; m++) {
            ...
            double bound = m == AR_CEILING ? log2(33.0) :
                           m == AR_WITNESSED_CEILING ? log2(1025.0) :
                           m == AR_FAST ? 10.0 : 16.0;
            assert(gain[m] >= -1.0 - 1e-12 && peak[m] - gain[m] <= bound + 1e-12);
            if (m != AR_CEILING && m != AR_WITNESSED_CEILING)
                assert(clipped[m] == 0);
        }
        ...
        double next_limit = state[AR_WITNESSED_CEILING].evidence <= -1.0 ? 5.0 : 10.0;
        assert(state[AR_WITNESSED_CEILING].odds <= next_limit);
        assert(state[AR_WITNESSED_CEILING].evidence == state[AR_WITNESS].evidence);
        assert(slow[AR_WITNESSED_CEILING] == slow[AR_WITNESS]);
        if (t == 0) {
            for (int m = 0; m < AR_MODES; m++)
            ...
            prior = exp2l((long double)before[AR_WITNESSED_CEILING].odds);
            posterior = prior * exp2l(delta);
            posterior /= 1.0L + posterior;
            hazard = slow[AR_WITNESSED_CEILING] ? 0x1p-16L : 0x1p-10L;
            after_hazard = (1.0L - hazard) * posterior;
            unbounded_odds = log2l(after_hazard / (1.0L - after_hazard));
            assert(clipped[AR_WITNESSED_CEILING] == (unbounded_odds > next_limit));
            assert(fabsl(state[AR_WITNESSED_CEILING].odds -
                         fminl(unbounded_odds, next_limit)) < 1e-10L);
        }
        clips += (unsigned)clipped[AR_CEILING];
        if (clipped[AR_WITNESSED_CEILING]) {
            if (next_limit == 5.0) low_clips++;
            else high_clips++;
        }
        if (matched == 0) {
            assert(state[AR_CEILING].evidence == before[AR_CEILING].evidence);
            if (delta == 0)
                assert(!clipped[AR_CEILING] &&
                       state[AR_CEILING].odds < before[AR_CEILING].odds);
    ...
    assert(clips && low_clips && high_clips && have_wrong &&
           have_fast_silence && have_slow_silence);
    printf("\n  ],\n  \"pass\":true,\n  \"clip_count\":%u,\n  \"maximum_binary_normalization_error\":%.17g,\n"
           "  \"witnessed_low_clips\":%u,\n  \"witnessed_high_clips\":%u,\n"
           "  \"sizeof_ARState\":%zu,\n  \"total_prediction_state_bytes\":%zu\n}\n",
           clips, max_norm, low_clips, high_clips, sizeof(ARState), sizeof(state));
    return 0;
}
