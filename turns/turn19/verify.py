#!/usr/bin/env python3
"""Independent turn13 source reader plus Decimal replay of five authority modes.
...
slow/fast/adaptive/witness/ceiling through every byte in Decimal mass arithmetic and
...
REPO = HERE.parents[1]
WORLDS = tuple(range(200, 208))
REGIMES = ('recombined', 'switched', 'moved_mid', 'unrelated')
MODES = ('slow', 'fast', 'adaptive', 'witness', 'ceiling')
CLOCK_MODES = ('adaptive', 'witness', 'ceiling')
TAIL_REGIMES = ('switched', 'moved_mid')
NAMESPACE = 'netta-commitment-ceiling-v1'
N = 16384
MOVE = 8192
WINDOW = 256
FAST_SHARE = 0.25
TOL = 1e-7
HORIZON = 32
COMMITMENT_CAP_BITS = 5
PROTOCOL_SHA = 'e00991a388a12b6ee3dbc803f1de613752c141944ed5ee30fe46ead7de53f71b'

spec = importlib.util.spec_from_file_location('turn19_independent_turn13_reader',
...
class Authority:
    ...
    def step(self, t, cold, candidate, matched):
        ...
        if self.active:
            ...
            elif self.mode in ('witness', 'ceiling') and matched >= 1:
        ...
        bound = math.log2(33) if self.mode == 'ceiling' else 10 if self.mode == 'fast' else 16
    ...
    with gzip.open(source, 'rt') as f, gzip.open(replay, 'rt') as g:
        ...
        for t in range(N):
            ...
            if regime in TAIL_REGIMES and t >= MOVE and common['active_before']:
                delta = float(row['ceiling_live'])-float(row['witness_live'])
...
def independent_gate(lives):
    ...
    def gap(regime, other):
        return [x-y for x, y in zip(read(regime, 'ceiling', 'tail'),
    ...
    early_ratio = (mean(read('recombined', 'ceiling', 'early'))/early_ref
                   if early_ref > 0 else None)
    full_ratio = (mean(read('recombined', 'ceiling', 'gain'))/full_ref
                  if full_ref > 0 else None)
    positive = read('recombined', 'ceiling', 'gain')
    first_fast = read('switched', 'ceiling', 'first_fast_after_move')
    offsets = [None if x is None else x-MOVE for x in first_fast]
    prompt = [x is not None and x < WINDOW for x in offsets]
    shares = read('moved_mid', 'ceiling', 'tail_fast_share')
    quiet = [x <= FAST_SHARE for x in shares]
    quantities = dict(
        moved_vs_fast=moved_fast, moved_vs_slow=moved_slow,
        law_vs_fast=law_fast, law_vs_slow=law_slow, law_vs_witness=law_witness,
    ...
    tests = dict(
        ...
        c3_law_vs_witness_mean=mean(law_witness) > 1,
        ...
        c7_drawdown_bound=all(s['max_drawdown'] <=
                              (math.log2(33) if mode == 'ceiling' else 10 if mode == 'fast' else 16)+TOL
        ...
        c9_cap_and_clock=all(life['exactness']['witness_clock_identical'] and
                            life['modes']['ceiling']['max_odds_before'] <= COMMITMENT_CAP_BITS and
                            life['modes']['ceiling']['max_odds_after'] <= COMMITMENT_CAP_BITS and
                            life['modes']['ceiling']['activation'] == life['modes']['witness']['activation']
                            for life in lives) and all(
                            lookup[w,'recombined']['modes']['ceiling']['clip_count'] > 0 for w in WORLDS))
    need(len(tests) == 21, 'twenty-one preregistered conditions')
...
def main():
    ...
    need(claim['window'] == WINDOW and claim['fast_share_bound'] == FAST_SHARE and
         claim['commitment_cap_bits'] == COMMITMENT_CAP_BITS,
    ...
    need(authority_count == 8*4*N*5, 'all authority forecasts')
    ...
    reader_tests = dict(
        ...
        c9_all_five_modes_replayed=authority_count == 8*4*N*5,
    ...
    result = dict(verification_pass=True, gate_pass=all(tests.values()),
                  ...
                  commitment_cap_bits=COMMITMENT_CAP_BITS,
                  ...
                  scope='Independent frozen turn13 source books and seven candidates; '
                        ...
                        'and ceiling authority, every quote, hazard choice, clip, odds and clock value. '
