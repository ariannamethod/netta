#!/usr/bin/env python3
"""One frozen source batch, four inherited controls and a five-bit commitment ceiling."""
import argparse
...
spec = importlib.util.spec_from_file_location('turn19_frozen_turn13_experiment',
...
FROZEN = (
    'turns/turn19/PROTOCOL.md', 'turns/turn19/experiment.py', 'turns/turn19/verify.py',
    'turns/turn19/authority.h', 'turns/turn19/authority.c', 'turns/turn19/replay.c',
    'turns/turn19/Makefile', 'turns/turn19/episode', 'turns/turn19/authority_replay',
...
def replay(world, regime, inherited):
    ...
    for t in range(N):
        ...
        for mode in MODES:
            ...
            if mode == 'ceiling':
                s['clip_count'] += int(row['ceiling_clipped'])
            s['gain'] += value-cold
            if t >= MOVE:
                s['tail'] += value-cold
        ...
        if regime in TAIL_REGIMES and t >= MOVE and active:
            delta = live['ceiling']-live['witness']
...
def summarize(lives):
    ...
    def pair(regime, other):
        return [a-b for a, b in zip(pick(regime, 'ceiling', 'tail'),
    ...
    quantities = dict(
        ...
        early_retention=avg(pick('recombined', 'ceiling', 'early'))/slow_early
            if slow_early > 0 else None,
        full_retention=avg(pick('recombined', 'ceiling', 'gain'))/slow_full
            if slow_full > 0 else None,
        recombined_full=pick('recombined', 'ceiling', 'gain'),
    ...
    tests = dict(
        ...
        c3_law_vs_witness_mean=avg(law_witness) > 1,
        ...
        c7_drawdown_bound=all(s['max_drawdown'] <=
            (math.log2(33) if m == 'ceiling' else 10 if m == 'fast' else 16)+TOL
        ...
        c9_cap_and_clock=all(x['exactness']['witness_clock_identical'] and
            x['modes']['ceiling']['max_odds_before'] <= 5.0 and
            x['modes']['ceiling']['max_odds_after'] <= 5.0 for x in lives) and
            all(by[w, 'recombined']['modes']['ceiling']['clip_count'] > 0 for w in WORLDS))
    assert len(tests) == 21
...
def evaluate():
    ...
    for item in inherited:
        ...
        lives.append(life)
        print(item['world'], item['regime'], 'ceiling',
              life['modes']['ceiling']['gain'], life['modes']['ceiling']['tail'],
              'first_fast_after_move',
              life['modes']['ceiling']['first_fast_after_move'],
              'tail_fast', life['modes']['ceiling']['tail_fast_count'], flush=True)
    ...
    result = dict(namespace=NAMESPACE, worlds=WORLDS, regimes=REGIMES, modes=MODES,
                  ...
                  fast_share_bound=FAST_SHARE, commitment_cap_bits=5,
