...
## Adaptation turn 11 — the Court-4 witness, first hand (2026-09-13)

The mycelium returned after Court 4, but the first adaptation contract had
two seams where a future power could have entered under the name of a fact.
The current branch did not contain a living mycelium at all: `main` had
removed the directory at `2859e5e`. Bodies 1–5 were recovered byte for byte
from their last complete merge, `535198b`. Turn 10 had added only the diet
and circulation preregistrations to this constitution; no body-6 code ever
existed. Those contracts remain visible as dead history rather than being
called a construction and silently repaired.

The second seam was identity. Court 4's `RelationKey` does not contain its
arm. The final book can carry the same key in the learned relation arm, the
oracle, and null controls. The adapter therefore names a candidate by
`arm=relation` plus all thirteen RelationKey fields; oracle and null rows
are witnesses only. The final sealed books contain 392 rows: 162 learned-arm
candidates and 230 controls. Five learned relations are live and five have
ever earned. The event book contains ten transitions, all EARN — five in
the learned arm and five in the oracle — and no REVOKE in this particular
world. The law still parses and tests REVOKE; absence in one biography is
not deletion from the grammar.

The audit also narrowed what can honestly be recomputed. Court 4's event
book contains threshold crossings, not every prospective-bit delta. A1 can
replay a citizen's admitted/revoked state and cross-check it against the
final relation row, but the final floating-point balance is witnessed from
the SHA-pinned relation book. Recomputing that balance would mean rebuilding
the Court-4 verifier path and rerunning the spent court, which this body is
forbidden to pretend it has done.

The portable capsule in `court4_biography/` is deliberately small: one
ordered manifest and byte-exact copies of the 903-byte transition book and
36,101-byte final relation book, 38,220 bytes total. The manifest preserves
the identities of roots2, base commitment, freeze, selection, builder
receipt, final C8 receipt, and the literal confirmatory verdict. Reports,
binaries, and large evidence stay in the ignored Netta closure archive.

Body A1 is adjacent to the old organism. `adaptation_witness.cpp` reads the
capsule as regular files, checks both SHA-256 identities, rebuilds the full
census, and seals the three exact input sections in one content-addressed
blob. Its two-row FNV ledger lives at `.mycelium.court4-witness`; repetition
is byte-idempotent. `adaptation_check.c`, a plain-C implementation sharing
no parser or hash code, reopens the blob and derives the same census. A1 has
no link to `mycelium.cpp` and no path to its grave, proposals, school,
parliament, notes, ranking, or mouth.

Thirteen first-hand gates are green: strict builds; the real capsule; the
independent census; idempotence; clean-room byte reproduction; a separately
pinned EARN-then-REVOKE transition probe in both hands; artifact, manifest and
symlink refusals; witness flip and truncation; a canonically resealed false
census; and a fully rehashed forged relation state. A sentinel court proves
all older body files remain byte-identical during the witness.
This is one author's finished hand, not independent social confirmation.
Don's counter-audit is the next turn; diet-v2 remains unbuilt and circulation
remains impossible.
...
was still wet: the second hand landed six findings on the roadmap before
any code existed, and the amendment commit folded every one in — the
attestation law renamed to what a stateless reader can prove, the
prototype's dead edges left buried, the ledger made event-sourced, the
dedup key moved from content to witness, the byte law declared, the
gate's refusal battery separated from the mechanism's ablation. One
refinement was named after the amendment and before code: the pinned
biography digest covers the attestation PREFIX — the biography's bytes
through the matched s line — because a whole-file digest would let the
same s event be eaten twice once the life lived further and the file's
bytes moved.

`mycelium.cpp` (C++17, stdlib only, strict build clean) ports the
resonance mechanics of microkarpathy's `mycelium.py` under the byte law
and adds the ledger and the gate; `ledger_check.c` is the independent
reader, plain C, no shared code. The battery ran in numbered rooms with
two lives raised on different worlds (Dracula and Frankenstein prefixes),
each speaking and signing its own stream.

The first battery room returned 12 pass, 1 fail — and the failure is the
log's favourite line: the preregistered ablation REFUSED its own organ.
At 3,600 lived bytes both lives spoke with the uni hand, and uni-order
byte soup carries no per-life lexical identity: metric A (top-1 clause
returns to its origin's life, origin excluded) sat at chance, 4/14 real
against 5/14 for the hash-vector control. The sealed prediction was
allowed to fail in public, which is the only reason it means anything.
One hypothesis — the input, not the organ — and the lives were raised to
27,600 lived bytes, where both mouths elected the bi hand and the speech
became word-shaped.

Second room, same sealed metric, untouched: 13 pass, 0 fail. Ablation
real A=60/98 (0.612) against control 44/98 (0.449), real B=0.2581
against 0.1938 — the resonance organ finds the life a clause came from
better than its absence does. The corpse of the standing prompt drew on
2/2 lives with full lineage. All four gate refusals (unsigned shuffle,
one flipped byte, re-ingest of an eaten pair, the same pair re-offered
through a grown biography) left the ledger byte-identical. Both hands —
writer replay and `ledger_check` — refused all three tamper classes
(blob content, ledger byte, missing blob) in the same verdict class, and
agreed on the honest chain to the last hex digit. A clean-room repeat of
the full sequence reproduced grave, ledger and stdout byte for byte.

Named limits, standing for the audit: the gate attests the pair, not
authorship — the state-witness is a future organ; a ledger prefix cut at
