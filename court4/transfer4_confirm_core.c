/* transfer4_confirm_core.c -- NETTA Court 4 confirmatory court core.
   Additive source root; transfer4.c remains the frozen development builder.
...
static double logmix2(double logp, double logq, double weight) {
    double a = weight < 1.0 ? log2(1.0 - weight) + logp : -INFINITY;
    double b = weight > 0.0 ? log2(weight) + logq : -INFINITY;
    return logadd2(a, b);
}

static double log1mexp2(double logp) {
    if (!(logp < 0.0)) die("log-domain event mass is not below one");
    return log2(-expm1(logp * log(2.0)));
}

static double log_floorU(const Model *M, uint32_t u) {
    return -8.0 * (double)M->exp_len[u] - M->log2_Z;
}

static double price_local_log2(const Model *M, uint32_t c1, uint32_t c2,
                               uint32_t truth) {
    double p1;
    if (M->n1_total > 0) {
        double seen = M->n1[truth]
            ? log2(1.0 - EPSILON) + log2((double)M->n1[truth])
              - log2((double)M->n1_total)
            : -INFINITY;
        p1 = logadd2(seen, log2(EPSILON) + log_floorU(M, truth));
    } else p1 = log_floorU(M, truth);
    ...
    double p2 = p1;
    {
        ...
        if (hi > lo) {
            ...
            for (size_t k = 0; k < nc; k++) {
                sum += ccbuf[k].cnt;
                if (ccbuf[k].tok == truth) wt = ccbuf[k].cnt;
            }
            double seen = wt > 0.0
                ? log2(1.0 - EPSILON) + log2(wt / sum) : -INFINITY;
            p2 = logadd2(seen, log2(EPSILON) + p1);
        }
    }
    ...
    double p = p2;
    {
        ...
        if (hi > lo) {
            ...
            double seen = wt > 0.0
                ? log2(1.0 - EPSILON) + log2(wt / sum) : -INFINITY;
            p = logadd2(seen, log2(EPSILON) + p2);
        }
    }
    if (!isfinite(p) || p > 1e-12)
        die("log-domain local price is outside the probability range");
    return p;
}
...
static void oracle_map(const uint8_t *omap, const int16_t learned_fwd[256],
    ...
    *matched = 0;
    for (int s = 0; s < 256; s++)
        if (learned_fwd[s] >= 0) {
            int d = alive_d[omap[s]] ? omap[s] : learned_fwd[s];
            fwd[s] = (int16_t)d;
            inv[d] = (int16_t)s;
            (*matched)++;
        }
...
static uint8_t *build_ff(const uint8_t *src, size_t n, size_t *out_n) {
    ...
    while (i < n) {
        ...
        if (is_ws(src[i])) {
            ...
            if (span < 64) for (int k = 0; k < 16; k++)
                if (ff_wlen[k] == span && !memcmp(ff_words[k], src + i, span)) {
                    hit = k;
                    break;
                }
            add = hit < 0 ? span : ff_wlen[(hit % 2 == 0) ? hit + 1 : hit - 1];
            i = j;
        }
    ...
    while (i < n) {
        ...
        int hit = -1;
        if (span < 64) for (int k = 0; k < 16; k++)
            if (ff_wlen[k] == span && !memcmp(ff_words[k], src + i, span)) { hit = k; break; }
        if (hit >= 0) {
            int partner = (hit % 2 == 0) ? hit + 1 : hit - 1;
            memcpy(w + o, ff_words[partner], ff_wlen[partner]);
            o += ff_wlen[partner];
        } else {
            memcpy(w + o, src + i, span);
            o += span;
        }
        i = j;
    }
...
static int outdir_fd = -1;
static FILE *open_out(const char *name) {
    char path[1200];
    FILE *f;
    if (outdir_fd >= 0) {
        struct stat st;
        int fd = openat(outdir_fd, name,
                        O_WRONLY | O_CREAT | O_EXCL | O_NOFOLLOW, 0600);
        if (fd < 0 || fstat(fd, &st) != 0 || !S_ISREG(st.st_mode)) {
            if (fd >= 0) close(fd);
            die("cannot create exclusive regular artifact");
        }
        f = fdopen(fd, "wb");
        if (!f) { close(fd); die("cannot stream exclusive artifact"); }
        return f;
    }
...
static double specialist_delta_log2(const Model *M, const FirstUnits *FU,
                                    uint32_t c1, uint32_t c2, uint32_t truth,
                                    const RelationKey *key) {
    double logpE = -INFINITY;
    uint8_t d = key->target_d;
    for (uint32_t i = FU->off[d]; i < FU->off[d + 1]; i++)
        logpE = logadd2(logpE,
                        price_local_log2(M, c1, c2, FU->unit[i]));
    if (!isfinite(logpE) || !(logpE < 0.0))
        die("log-domain specialist event outside (0,1)");
    double pv[256];
    unit_start_ps_vec(key->ctx_s, key->cl, pv);
    double r = pv[key->target_s];
    if (!(r > 0.0 && r < 1.0))
        die("specialist source event outside (0,1)");
    const uint8_t *tb = M->pool + M->exp_off[truth];
    return tb[0] == d ? log2(r) - logpE
                      : log2(1.0 - r) - log1mexp2(logpE);
}
...
static double relation_price_and_update(Arm *A, int arm_index,
    ...
    for (int i = 0; i < n; i++)
        delta[i] = specialist_delta_log2(M, FU, c1, c2, truth,
                                         &active[i]->key);

    double logpfin = logpl;
    if (winner) {
        double logqtruth = logpl + delta[winner_at];
        logpfin = logmix2(logpl, logqtruth, winner->L);
        if (!isfinite(logpfin) || logpfin > 1e-12)
            die("log-domain live relation price outside probability range");
        stats->winner_uses++;
        A->winner_uses++;
...
static void surface_shadow_update(RelationBook books[2], const Model *M,
    ...
    for (int b = 0; b < 2; b++) {
        ...
        double delta = specialist_delta_log2(M, FU, c1, c2, truth, &key);
...
static double downstream_fixture_step(Arm arms[NARMS], const Model *M,
                                      ...
                                      FILE *winner_out, FILE *event_out) {
    double logpfin[NARMS] = {0};
    ChunkStats stats[NARMS];
    memset(stats, 0, sizeof(stats));
    for (int a = ARM_REL; a < ARM_ORACLE; a++)
        logpfin[a] = relation_price_and_update(&arms[a], a, M, FU,
                                            UINT32_MAX, UINT32_MAX,
                                            truth, log2(pl), W, bp, step, &stats[a],
                                            winner_out, event_out);
    for (int k = 0; k < NULL_K; k++) {
        int a = ARM_NULL0 + k;
        if (!same_double(logpfin[ARM_REL], logpfin[a]))
...
static void run_court(const char *wname, const uint8_t *W, size_t Wn_full,
    ...
    while (lived < Wn) {
        ...
        for (size_t pos = 0; pos < npc; pos++) {
            ...
            double logpl = price_local_log2(&M, c1, c2, truth);
            bits[ARM_COLD] += -logpl;
            if (check_surface)
            ...
            for (int a = 1; a < NARMS; a++) {
                if (a == ARM_ORACLE && !has_oracle) continue;
                double logpfin = relation_price_and_update(&arms[a], a, &M, &FU,
                                                        c1, c2, truth, logpl, W,
                                                        byte_off,
                                                        priced_units + pos,
                                                        &cs[a],
...
int transfer4_confirm_core_legacy_main(int argc, char **argv) {
    check_null_seeds();
...
static void confirm_core_prepare_source(const uint8_t *A, size_t an) {
    size_t atrain = (an * 9) / 10;
    cargo_build(A, atrain);
    for (int s = 0; s < 256; s++) source_first[s] = UINT64_MAX;
    for (size_t i = 0; i < atrain; i++)
        if (source_first[A[i]] == UINT64_MAX)
            source_first[A[i]] = (uint64_t)i;
    Model source_units;
    memset(&source_units, 0, sizeof(source_units));
    model_build(&source_units, A, atrain);
    unit_start_build(A, atrain, &source_units);
    ...
    fprintf(stderr, "confirm source: A-train %zu B | unit starts %llu\n",
            atrain, (unsigned long long)us1tot);
}
...
void transfer4_confirm_core_run(const uint8_t *source, size_t source_bytes,
                                ...
                                double G[22][5]) {
    struct stat st;
    if (world_bytes != RUN_BYTES)
        die("confirmatory core world length mismatch");
    if (output_dir_fd < 0 || fstat(output_dir_fd, &st) != 0 ||
        !S_ISDIR(st.st_mode))
        die("confirmatory core requires a held output directory");
    if (outdir_fd >= 0)
        die("confirmatory core output directory already active");
    check_null_seeds();
    confirm_core_prepare_source(source, source_bytes);
    outdir_fd = output_dir_fd;
    run_court("confirmatory", world, world_bytes, oracle, 0, 0,
              G, RUN_BYTES, NULL);
    outdir_fd = -1;
    free(us2k); free(us3k); free(us4k);
    us2k = us3k = NULL;
